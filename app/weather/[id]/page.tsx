    ...
    return (
        <main className="flex min-h-screen flex-col items-start p-20">
            <div className='flex flex-col items-start justify-between'>
                <h1 className='font-extralight text-3xl md:text-5xl'>{`${data[0].state}, ${data[0].country}`}</h1>
                <h2 className='font-bold text-2xl md:text-4xl mt-2 md:mt-4'>{`${Math.round(temperatures.main.temp)}°C`}</h2>
                <h3 className='font-bold text-2xl md:text-4xl mt-2 md:mt-4'>{`${temperatures.weather[0].main}`}</h3>
                <div className='flex justify-center items-center mt-5'>
                    ...
                    <div className='ml-3'><CloudIcon className='inline h-6 mr-2'></CloudIcon> <span className='inline'>{`${Math.round(temperatures.main.humidity)}%`}</span></div>
                </div>
            </div>
        </main>
    );
